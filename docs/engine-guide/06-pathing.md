...
Pillar 1 says the maze is the game, so the walkable grid changes constantly and every visitor must
re-route. Gridfall does this with **one flow field**, not per-visitor pathfinding. See
...
2. **`if (_dist[n] != ushort.MaxValue) continue;`** — first assignment wins. A cell reached again at
   ...
   chosen route depends on the frontier shape rather than the stated rule, and visitors split across
...
than O(visitors) costs nothing on the 99% of ticks where nothing was built.
...
{
    var cell = state.VisitorCell[slot];
    byte dir  = _path.FlowAt(cell);
    if (dir == Unreachable) { /* stand still — see below */ }
    else state.VisitorHeading[slot] = dir;
}
```

A visitor reads the field **only when it crosses into a new cell**. Between cells it keeps its heading no
matter what phase 2 did. That is what makes "no visitor turns mid-cell" true without any extra state, and
it is a design rule the player learns, not a limitation being hidden.

### Unreachable cells

The block check makes a fully sealed lane impossible, so a visitor should never stand on an unreachable
...
The defined behavior is: **stand still and emit `VisitorStranded` once.** Not "walk toward the goal
...
| Per-visitor query | One array read | Only on a cell boundary crossing |

Cost is **independent of visitor count**. 300 visitors and 1 visitor cost the same, which is the property
...
Per-unit path variation — flyers, or visitors that avoid a specific station — needs **another whole field**,
not a per-visitor tweak. That is 24 KB and one more O(cells) pass per dirty tick, per variant.

This is a real constraint on future design, and it is the main thing ADR-0003 forecloses. Two or three
fields is fine. A field per visitor archetype is not, and the design should hear that before it plans a
game around it.
