...
This area names knobs; it does not set their values. `frost-spire slows visitors` is design.
`frost-spire slows visitors by 35%` is `content-data`.
...
| Feature design spec | the requirements file, `docs/pillars.md`, `features/*` for adjacent features | `../docs/iso-grid.md`, `../docs/tech-standards.md`, wave/station JSON |
...
- Don't design the implementation. "Visitors re-path when the maze changes" is yours; flow fields are not.
