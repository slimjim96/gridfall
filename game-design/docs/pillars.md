...
Stations are not turrets bolted to a fixed path. Placing a station **changes where visitors walk**. The
...
*Implies:* no two visitor archetypes share a silhouette; every player-visible state has a visible
...
When a visitor leaks, the player can point at the reason. Not "the difficulty ramped" — *that* visitor, on
...
Few stations, few visitors, deep interactions. Gridfall would rather have eight stations whose combinations
matter than forty that differ by a stat line.

*Implies:* a new station must justify itself against the two it most resembles. "It's a faster version of
...
|---|---|
| The maze is the game | Fixed lanes, unblockable paths, stations that only shoot |
...
| Small numbers, big decisions | The ninth station that is the third station with more DPS |
